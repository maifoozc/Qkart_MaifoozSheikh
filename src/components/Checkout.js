...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    //balance check
    let sub_Balance = parseInt(localStorage.getItem("balance"));

    if (getTotalCartValue(items) > sub_Balance) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        {
          variant: "warning",
  ...
  const performCheckout = async (token, items, addresses) => {
    if (validateRequest(items, addresses)) {
      let addressId = addresses.all.find((e) => {
        return e.address === addresses.selected;
      });
  ...
  const selectAddress = (e) => {
    ...
    setAddresses({ ...addresses, selected: e.address });
  };
  ...
  useEffect(() => {
    if (!token) {
      ...
      console.log(addresses.all, "stored addresses");
      console.log(token);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token]);
                ...
                addresses.all.map((item) => (
                  <Box
                    ...
                    className={`address-item ${
                      addresses.selected === item.selected
                        ? "selected"
                        : "not-selected"
                    }`}
                    onClick={() => {
